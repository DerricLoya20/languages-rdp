...
public:
  ...
  virtual Token::Ptr nextFromStream() override
  {
    ...
    if (g.ch == 'S' || g.ch == 'R')
      return Token::keyword(std::string(1, char(g.ch)), g.line, g.col);
    if (g.ch == '+')
      return Token::add(g.line, g.col);
    if (g.ch == '-')
      return Token::sub(g.line, g.col);
    if (g.ch == '*')
      return Token::times(g.line, g.col);
    if (g.ch == '/')
      return Token::divide(g.line, g.col);
    if (g.ch == '(')
    ...
    if (g.ch == -1)
      return Token::eof(g.line, g.col);
    if (g.ch == '?')
      return Token::eoe(g.line, g.col);
